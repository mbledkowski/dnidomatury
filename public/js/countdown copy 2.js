...
var countdown = (
  function (module) {
    ...
    function Timespan() { } function rippleRounded(ts, toUnit) {
      switch (toUnit) {
        ...
        case 'years':
          ...
          ts.years = 0;        /* falls through */
        case 'decades':
          if (ts.decades !== DECADES_PER_CENTURY || isNaN(ts.centuries)) {
            return;
          } ts.centuries++;
          ts.decades = 0;        /* falls through */
        case 'centuries':
    ...
    function fractional(ts, digits) {
      ...
      if (!frac) { return; } frac = fraction(ts, frac, 'years', 'decades', YEARS_PER_DECADE, digits);
      if (!frac) { return; } frac = fraction(ts, frac, 'decades', 'centuries', DECADES_PER_CENTURY, digits);
      if (!frac) { return; } frac = fraction(ts, frac, 'centuries', 'millennia', CENTURIES_PER_MILLENNIUM, digits);
    ...
    function ripple(ts) {
      ...
      if (ts.years >= YEARS_PER_DECADE) {
        ...
        ts.years %= YEARS_PER_DECADE; if (ts.decades >= DECADES_PER_CENTURY) {
          ts.centuries += floor(ts.decades / DECADES_PER_CENTURY);
          ts.decades %= DECADES_PER_CENTURY; if (ts.centuries >= CENTURIES_PER_MILLENNIUM) {
